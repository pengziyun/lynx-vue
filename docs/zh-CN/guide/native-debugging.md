...
- `processData is not a function` / `renderPage is not a function`：主线程脚本没有正确完成 bootstrap。重新构建 native bundle，确认上面的浏览器 runtime 标记都已经消失，删除 Lynx Explorer 里的旧卡片，再重新扫码。
